...
def train_full(model, decode_mode, train_loader, val_loader, optimizer, scheduler, loss_func, n_epochs, device,
    ...
    nse_log2 = BestModelLog(model, saving_root, "max_nse_train", high_better=True)
    # newest_log = BestModelLog(model, saving_root, "newest", high_better=True)
    epoch_log = BestModelLog(model, saving_root, "epoch", high_better=True, log_all=False)
    ...
    for i in range(n_epochs):
        ...
        mse_train, nse_train = eval_model(model, train_loader, decode_mode, device)  # TODO:cross的情况下，train和vali一样
        ...
        nse_log2.update(model, nse_train, i)
        epoch_log.update(model, i, i)
    t2 = time.time()
